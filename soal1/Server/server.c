...
#include <sys/stat.h>
#include <sys/time.h>
#define S_PATH "/home/vicky/praktikum/modul3/soal1/Server/FILES/"
...
void append_account(const char *id, const char *password) {
...
int main () {
    ...
    int server_fd, new_fd, isServing=1, i=0,
		return_value, return_val1, return_val2, return_val3, status_value, connections[10];
        ...
        if (return_value >= 0 ) {
                ...
                if (new_fd >= 0) {
                    ...
                    for (i=0; i < 10; i++) {
                        if (connections[i] < 0) {
                            ...
                            if(i != isServing)  return_val1 = send(connections[i], "wait",  100, 0);
                            else return_val1 = send(connections[i], "serve",  100, 0);
                            break;
                        }
                    }
            ...
            for (i=1; i<10; i++) {
                if ((connections[i]>0) && (FD_ISSET(connections[i], &read_fd_set))) {
                    return_val1 = recv(connections[i], command, sizeof(command), 0);
                    ...
                    if (return_val1) {
                        ...
                        if(!strcmp(command, "register")) {
                            if(login) continue;
                            return_val2 = recv(connections[i], id, sizeof(id), 0);
                            return_val3 = recv(connections[i], password, sizeof(password), 0);
                            if(get_account(0, id, password)) {
                                status_value = send(connections[isServing], "user_exists", 100, 0);
                            } else {
                                login = 1;
                                append_account(id, password);
                            ...
                            if(login) {
                                ...
                                if(!strcmp(command, "add")) addFeature(connections[isServing], userdata);
                                if(!strcmp(command, "download")) downloadFeature(connections[isServing]);
                                if(!strcmp(command, "delete")) deleteFeature(connections[isServing], userdata);
                                if(!strcmp(command, "see")) seeFeature(connections[isServing]);
                                if(!strcmp(command, "find")) findFeature(connections[isServing]);
                            } else {
                    ...
                    if (return_val1 == -1 || return_val2 == -1 || return_val3 == -1) {
                        printf("Receiving failed for fd: %d\n", connections[i]);
                        break;
                    }
                }
                return_val1--;
                if (!return_val1) continue;
